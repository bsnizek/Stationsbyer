...
def search(geo='', id='',name='', xmin='',ymin='',xmax='',ymax='',sw=None):
    # coords = geo.split(',')
    # query = "http://api.wikilocation.org/articles?lat=%s&lng=%s&radius=5000" % (coords[0], coords[1])
    query = "http://www.panoramio.com/map/get_panoramas.php?order=popularity&set=public&from=0&to=10000&minx=%s&miny=%s&maxx=%s&maxy=%s" % (ymin,xmin,ymax,xmax)
    print query
...
if __name__ == '__main__':
    ...
    for city in CITIES:
        ...
        #query Search API
        search(geo=geo, id=id, name=name, xmin=xmin, ymin=ymin, xmax = xmax, ymax = ymax, sw=sw)
